...
### 5.1 图片加载组件 ImageWithFallback

项目提供了带回退功能的图片组件，自动处理图片加载失败的情况。

**组件位置：** `src/app/[locale]/ImageWithFallback.tsx`
...
function ArticleCard({ article }) {
  return (
    <ImageWithFallback
      src={article.coverImage}          // 图片URL
      alt={article.title}                // Alt文本
...
export default function ImageWithFallback({ src, alt, ...props }) {
  const fallbackSrc = '/default-cover.jpg'
...
**为什么需要这个组件？**
- Server Component 不能使用 `onError` 等事件处理器
- 需要在客户端处理图片加载失败的情况
- 提供更好的用户体验
