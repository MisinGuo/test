...
function patchCode(code) {
  ...
  //    ESA 的 ExecutionContext(ctx，fetch 第 3 参数) 可能为 undefined 或不含这些方法
  //    改为可选链，避免 "Cannot read properties of undefined" 崩溃
  code = code.replace(/\.waitUntil\(/g, '?.waitUntil?.(')
  code = code.replace(/\.passThroughOnException\(/g, '?.passThroughOnException?.(')
